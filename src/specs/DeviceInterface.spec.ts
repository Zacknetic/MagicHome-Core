...
describe('Test the DeviceInterface class', () => {
    before(done => {
        setTimeout(done, 100);
        transport = new Transport('192.168.1.19');
    ...
    it('Should set light state to red', async function () {
        let deviceCommand = { isOn: true, RGB: { red: 0, green: 0, blue: 100 }, CCT: { warmWhite: 0, coldWhite: 0 }, colorMask: 0xF0 };
        const commandOptions: ICommandOptions = { colorAssist: false, waitForResponse: true, maxRetries: 2, remainingRetries: 2, commandType: COLOR_COMMAND, timeoutMS: 50, isEightByteProtocol: true };
        ...
        //     })
        // await sleep(500);
        deviceCommand = { isOn: true, RGB: { red: 100, green: 0, blue: 0 }, CCT: { warmWhite: 100, coldWhite: 0 }, colorMask: 0x00 };
